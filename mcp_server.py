...
app itself (`python -m hypatia`, detached, on the port of HYPATIA_URL) and waits
for it, so a workspace that starts before the app still gets its tools;
HYPATIA_BRIDGE_AUTOSTART=0 turns that off.
"""
...
def _healthy() -> bool:
    try:
        ...
        return response.status_code == 200 and response.json().get("service") == "hypatia-hoard"
    except Exception:
        return False
...
def ensure_running(timeout_s: float = 45.0) -> bool:
    ...
    if _healthy():
        return True
    if os.environ.get("HYPATIA_BRIDGE_AUTOSTART", "1") == "0":
        return False
...
def _token() -> str:
    env = os.environ.get("HYPATIA_TOKEN")
    if env:
        return env.strip()
    return TOKEN_FILE.read_text(encoding="utf-8").strip()


class HypatiaBridge(FastMCP):
    ...
    def __init__(self, catalog: list[dict], instructions: str):
        super().__init__(name="hypatia-hoard", instructions=instructions)
...
def main() -> None:
    ...
    HypatiaBridge(catalog, instructions).run(transport="stdio")
